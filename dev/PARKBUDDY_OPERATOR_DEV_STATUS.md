...
## 2026-06-14 Public Member Card Density
- 일반 회원 목록 카드의 내부 여백과 카드 간격을 줄여 모바일 한 화면 정보 밀도를 개선했다.
- 연락처는 하단 전체 폭을 유지해 한 줄 표시와 전화 연결 동작을 보존했다.
- 회원 상세 팝업 동작은 유지하고 목록 카드의 표시 밀도만 조정했다.

## 2026-06-14 Admin Member Edit UX Polish
- 회원관리 목록에서 회원명 링크와 중복되는 수정 버튼을 제거했다.
- 회원 수정 화면의 상단 목록 버튼을 제거하고, 하단 액션을 수정 저장 / 취소 순서의 모바일 가로 2열로 정리했다.
- 회원 수정 연락처 입력값을 하이픈 포함 형식으로 표시하도록 정리했다.
