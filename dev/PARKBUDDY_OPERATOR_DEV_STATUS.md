...
## 2026-06-16 추가 기록 - 인증 고도화 패스키 준비 점검

작업명: `PARKBUDDY_AUTH_PASSKEY_READINESS`

확인 내용:

- ParkBuddy 인증 방향을 `첫 접속: 카카오 또는 운영자 초대 코드`, `다음 접속: 지문/Face ID/패스키`로 유지한다.
- SMS OTP는 비용 문제로 제외한다.
- 이메일 매직링크는 주 로그인으로 쓰지 않고, 필요 시 관리자/비상 복구용으로만 검토한다.
- Supabase native passkey는 experimental이며 클라이언트 opt-in과 Dashboard 설정이 필요하다.
- Supabase native passkey 등록은 confirmed non-anonymous 사용자가 필요하고 SSO 사용자는 제한이 있어, Kakao-first 흐름에 바로 붙이는 것은 보류한다.
- 다음 실제 개발은 `PARKBUDDY_AUTH_INVITE_CODE_ACCOUNT_FLOW`로 잡고, 운영자 발급 초대 코드 또는 로그인 ID 기반 첫 접속 구조부터 설계한다.

다음 우선순위:

```text
1. 운영자 발급 초대 코드/로그인 ID 기반 첫 접속 설계
2. Supabase Auth와 연결 가능한 non-SSO 계정 흐름 결정
3. 로그인 화면 모바일 가용성 안내 보강
4. passkey 등록/로그인 구현
5. 라운딩 관리 화면에 연결된 일정 정보 표시 재개
```
