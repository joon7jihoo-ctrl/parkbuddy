...
## 2026-06-15 - 스코어 입력 태블릿/데스크탑 입력창 배치 수정

- 대상: `src/components/admin/round-score-input-form.tsx`
- 모바일 배치는 유지하고, `sm` 이상 화면에서 회원명 / 총 타수 / 스테이블포드 포인트가 한 줄 3열로 배치되도록 수정했다.
- 입력 label에 `min-w-0`을 추가해 태블릿/데스크탑에서 포인트 입력창이 회원명 아래로 밀리지 않도록 했다.
- 검증: `npm run verify` 통과.
- 상세 기록: `dev/PARKBUDDY_SCORE_INPUT_TABLET_LAYOUT_FIX.md`

## 2026-06-15 - Score input fluid width fix

- 태블릿/데스크탑 스코어 입력 카드에서 `총 타수`, `스테이블포드 포인트` 입력창이 화면 너비에 맞춰 늘어나도록 grid 컬럼을 수정했다.
- 모바일 배치와 미입력 필터 동작은 변경하지 않았다.
- 확인: `npm run verify`


## 2026-06-15 스코어 입력 반응형 그리드 보정

- 태블릿/데스크탑에서 회원 스코어 카드를 2열 또는 3열로 배치하도록 수정했다.
- 모바일이 아닌 화면에서도 일자/참가/입력 요약 카드가 사라지지 않도록 수정했다.
- 태블릿/데스크탑에서는 미입력만 보기 버튼을 입력 카드 오른쪽에 배치했다.
- 모바일에서는 기존처럼 요약 카드 아래에 미입력만 보기 버튼이 표시된다.
- 스코어 저장 로직과 Supabase 관련 코드는 변경하지 않았다.
- 검증: `npm run verify` 통과.

## 2026-06-15 - 스코어 입력 카드 정렬 보정

- 모바일/태블릿/데스크탑에서 회원 카드 내부 배치를 동일하게 정리했다.
- 회원명, 핸디캡, 입력상태가 세로로 보이도록 조정했다.
- 총 타수 입력 폭을 줄이고 스테이블포드 포인트 입력 폭을 늘려 라벨 줄바꿈을 최소화했다.
- 태블릿/데스크탑 상단 요약 영역의 일자/참가/입력/미입력만 보기 폭을 균등하게 맞췄다.
- 상세 기록: `dev/PARKBUDDY_SCORE_INPUT_CARD_ALIGNMENT_FIX.md`
