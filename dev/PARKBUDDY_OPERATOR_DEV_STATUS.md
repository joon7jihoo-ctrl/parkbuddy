...
## 추가 기록: 인증 전략 조정 - 패스키 + 초대 코드

작성일: 2026-06-16

모바일 데이터망에서 카카오 로그인 중 네트워크 검증 오류가 발생할 수 있고, 이를 ParkBuddy 앱 쪽에서 초기화하거나 제어하기 어렵다는 점을 확인했다. 같은 휴대폰을 PC와 동일한 Wi-Fi에 연결하면 정상 접속되는 사례가 확인되었으므로, ParkBuddy/Vercel/Supabase 기본 연결 문제라기보다 카카오 인증 과정의 네트워크 검증 이슈로 판단한다.

SMS OTP는 발송 비용 때문에 제외한다. 이메일 매직링크는 파크골프 이용자 연령대를 고려할 때 주 로그인 방식으로는 불편할 수 있으므로 비상 복구용으로만 검토한다.
...
보조 로그인: 카카오 유지
비상 복구: 운영자 초대 코드 재발급, 필요 시 이메일은 관리자/비상 복구용
```

추가 문서:

```text
dev/PARKBUDDY_AUTH_STRATEGY_PASSKEY_INVITE.md
```
...
2. 운영 도메인/RP ID 확정
3. PASSKEY_READINESS 작업
4. 로그인 후 패스키 등록 UX
5. 패스키 로그인 버튼 추가
6. 초대 코드 기반 백업 첫 로그인 구현
```

주의: Supabase passkey 등록은 기존 confirmed non-anonymous auth user가 필요하므로, 초대 코드만으로 passkey를 등록하려면 먼저 안전한 Supabase Auth 세션 생성 방식이 필요하다.
