...
## 2026-06-17 - Auth Invite Code Account Flow

### 목적
카카오 로그인이 일부 모바일 데이터망에서 실패해도 사용자가 SMS/이메일 없이 ParkBuddy에 첫 접속할 수 있도록 로그인 화면에 운영자 발급 초대 코드 흐름을 추가했다.

### 변경 사항
- `src/app/(auth)/login/page.tsx`를 서버 컴포넌트로 전환했다.
- 카카오 OAuth 버튼을 `src/components/auth/kakao-login-button.tsx` 클라이언트 컴포넌트로 분리했다.
- `/login?invite=1`에서 이름, 연락처, 초대 코드 입력 폼을 제공한다.
- 세션이 없는 사용자는 Supabase anonymous sign-in 후 기존 `claim_member_account` RPC로 회원 계정을 연결한다.
- 초대 코드 실패/만료/RPC 누락/Anonymous 비활성화 오류를 사용자 메시지로 분리했다.
...
### 운영 전 확인
- Supabase Dashboard에서 Anonymous sign-ins 활성화 필요
- 기존 `claim_member_account` RPC 배포 필요
- 회원 관리 화면에서 연결 코드 발급/재발급 후 실제 모바일 테스트 필요

### 다음 단계
1. 운영 Supabase Anonymous sign-in 설정 확인
2. 모바일 데이터망에서 초대 코드 로그인 테스트
3. 장기 세션 유지와 passkey 등록 UX 설계로 이동
