...
## 2026-06-16 - Vercel Production Deployment 기준 정리

Status: completed

Summary:

- GitHub Pages 운영 배포 시도는 ParkBuddy의 Next.js 동적 라우트/Middleware 구조와 맞지 않아 Vercel 운영 배포 기준으로 전환했다.
- Vercel Production Branch Tracking을 `feature/operator-stability-and-round-safety`로 설정했다.
- 운영 도메인 `https://parkbuddy-five.vercel.app` 기준으로 모바일/데스크탑 테스트를 진행한다.
- 새 push 후 Vercel Production 배포가 생성되는지 확인하는 절차를 정리했다.
- 자동 배포가 감지되지 않는 경우의 수동 배포/CLI 배포 절차를 문서화했다.
- Vercel 환경변수와 Supabase Redirect URL 확인 항목을 고정했다.

Artifacts:

- `dev/PARKBUDDY_VERCEL_PRODUCTION_DEPLOYMENT.md`

Next:

- 다음 기능 작업부터는 `npm run verify → git commit → git push → Vercel Production 배포 확인 → 모바일/데스크탑 화면 캡처` 순서로 진행한다.
