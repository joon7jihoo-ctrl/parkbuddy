...
## 2026-06-13 라운드 삭제/복구 UI 최종 정리 계획

### 작업 기준

- 브랜치: feature/operator-stability-and-round-safety
- 기준 커밋: 161938d Remove old round id delete list
- 작업 트리: clean 상태에서 시작

### 개발 방향

최초 개발가이드와 현재 운영 안정화 보고서를 기준으로, 라운드 삭제/복구 기능은 기능 추가보다 데이터 보존성과 운영 실수 방지를 우선한다. 라운드는 실제 삭제하지 않고 soft delete 처리하며, 참가자, 조 편성, 스코어, 결과 데이터는 유지한다.

### 이번 패치 범위

- 기본 라운드 목록에서 삭제 성공 안내 메시지를 표시한다.
- 기본 라운드 목록 하단에 남아 있던 빈 삭제 영역을 제거한다.
- 삭제된 라운드 화면의 제목과 안내 문구를 삭제/복구 흐름에 맞게 정리한다.
- 삭제된 라운드 화면에서는 일반 운영 버튼을 제거하고 복구 버튼만 제공한다.
- 삭제/복구 후 /admin/rounds, /admin/rounds/deleted, /admin/logs를 다시 검증할 수 있도록 revalidatePath와 redirect 흐름을 정리한다.

### 검증 기준

1. npm run verify 통과
2. /admin/rounds 하단에 빈 빨간 박스가 없어야 함
3. /admin/rounds에서 라운드 삭제 후 기본 목록에서 사라져야 함
4. /admin/rounds/deleted에서 삭제된 라운드만 보여야 함
5. /admin/rounds/deleted에서 참가자 선택, 조 편성, 스코어 입력, 결과 보기, 수정, 상태 변경, 라운드 복제 버튼이 보이지 않아야 함
6. 삭제된 라운드 복구 후 기본 목록에 다시 표시되어야 함
7. /admin/logs에서 삭제/복구 로그를 확인해야 함


## 2026-06-14 Admin log Korean label update

- 관리자 작업 로그에서 라운드 삭제 보관/복구 action 코드가 영어로 보이지 않도록 한글 라벨을 추가한다.
- 대상 파일: `src/app/(app)/admin/logs/page.tsx`, `src/app/(app)/admin/page.tsx`, `src/app/(app)/admin/members/page.tsx`
- 라운드 soft delete 메타데이터의 `round_title`, `deleted_at`, `deleted_by_member_id` 표시명을 한글화한다.
- 검증 기준: `npm run verify` 통과 후 `/admin/logs`와 `/admin` 최근 작업에서 `round_soft_delete`, `round_restore`가 각각 `라운드 삭제 보관`, `라운드 복구`로 보이는지 확인한다.

## 2026-06-14 Round safety permission audit

- 라운드 삭제/복구 UI 및 로그 한글화 커밋 이후 권한/RPC 1차 정적 점검 보고서를 추가했다.
- 보고서: `dev/PARKBUDDY_ROUND_SAFETY_PERMISSION_AUDIT.md`
- 1차 결론: 삭제/복구 RPC는 앱/DB 양쪽에서 관리자 및 동호회 검증을 갖춘 구조로 보인다.
- 추가 보완 필요: 일부 라운드 상세/운영 경로에서 `deleted_at is null` 필터 또는 삭제 라운드 접근 차단 여부를 확인해야 한다.
- 다음 권장 작업: `Hide deleted rounds from operational routes`

## 2026-06-14 - 삭제된 라운드 일반 운영 화면 접근 차단


- 작업명: Hide deleted rounds from operational routes
- 목적: soft delete 된 라운드가 일반 운영 화면에서 수정/스코어/조 편성/결과 작업으로 이어지지 않도록 차단한다.
- 추가 컴포넌트: src/components/admin/deleted-round-operation-blocked.tsx
- 직접 라운드 운영 화면에는 삭제된 라운드 안내 UI를 표시한다.
- 라운드 캘린더, 상태 필터, 회원 스코어 목록에는 deleted_at is null 조건을 추가한다.
- 라운드 수정 액션 및 조 편성 메타 업데이트 액션도 삭제된 라운드에 적용되지 않도록 방어 조건을 추가한다.
- 검증 기준: npm run verify 통과 후, 삭제된 라운드의 참가자/조 편성/스코어/결과/수정 URL 접근 시 복구 안내 화면이 보여야 한다.

## 2026-06-14 - 라운드 삭제/복구 RPC 권한 보강 SQL 추가

### 목표

라운드 삭제/복구 UI 차단 이후, Supabase RPC 실행 권한을 DB 관점에서 다시 점검한다.
...
1. Supabase SQL Editor에서 `parkbuddy_round_safety_rpc_permission_check.sql` 실행
2. PUBLIC 또는 anon execute 권한이 true이면 `parkbuddy_harden_round_safety_rpc_permissions.sql` 실행
3. 점검 SQL 재실행
4. 최종 결과가 PASS인지 확인
5. 로컬에서 `npm run verify` 실행

### 완료 기준

- `admin_soft_delete_round(uuid)`가 authenticated만 실행 가능
- `admin_restore_round(uuid)`가 authenticated만 실행 가능
- 두 함수 모두 내부에서 active member, same club, admin role을 검증
- 운영자 계정의 삭제/복구 기능은 정상 동작
- 일반 회원/익명 사용자는 삭제/복구 불가

## 2026-06-14 Admin mobile navigation and safety UI polish

- 목표: 운영자 화면의 위험 작업 UX를 모바일 우선으로 정리하고, 화면이 커져도 앱 하단 네비게이션이 사라지지 않도록 수정한다.
- 반영 파일:
...
- 수정 방향:
  - 하단 네비게이션의 md:hidden 동작을 제거하고, 모든 화면 크기에서 하단 앱 네비게이션이 보이게 한다.
  - 큰 화면에서는 전체 폭을 차지하지 않고 중앙 하단 플로팅 dock처럼 보이게 하여 버튼 위치가 어색하지 않도록 한다.
  - 모바일에서 스크롤을 줄이기 위해 관리자 대시보드의 간격, 카드 padding, 버튼 배치를 더 압축한다.
  - 운영자 핵심 버튼은 모바일에서 3열로 배치하여 한 화면 안에서 주요 이동 경로를 빠르게 누를 수 있게 한다.
- 검증 기준:
  - npm run verify 통과
  - 모바일 폭에서 하단 네비게이션 표시
  - 태블릿/PC 폭에서도 하단 네비게이션 표시
  - 인쇄 결과표 화면에서는 하단 네비게이션 숨김 유지
  - /admin 첫 화면의 카드와 버튼 간격이 과하게 늘어지지 않는지 확인



---

## PARKBUDDY_ROUNDS_MOBILE_LAYOUT_POLISH_2026_06_14

- 작업명: Polish round management mobile layout
- 목적: 라운드 관리 화면을 모바일 우선으로 재배치하고, 데스크탑에서도 라운드 정보가 좁게 눌리지 않도록 개선한다.
- 주요 변경:
  - /admin/rounds 최대 폭을 넓혀 데스크탑 가독성을 개선한다.
  - 라운드 기본 정보를 모바일 2열 카드형 그리드로 배치한다.
  - 핵심 작업 버튼은 참가자, 조 편성, 스코어, 결과만 우선 노출한다.
  - 수정, 복제, 상태 변경은 더보기 접기/펼치기 영역으로 이동한다.
  - 삭제는 위험 작업 접기/펼치기 영역으로 분리한다.
  - 메모는 접기/펼치기로 표시해 스크롤을 줄인다.
- 검증 기준:
  - npm run verify 통과
  - 데스크탑 폭에서 라운드 정보가 좁게 눌리지 않음
  - 모바일 폭에서 버튼이 무조건 세로로 길게 쌓이지 않음
  - 삭제/복구 안전 흐름 유지


---

## PARKBUDDY_ROUND_MANAGEMENT_LAYOUT_STANDARD_2026_06_14

- 작업명: Apply consistent round management layouts
- 목적: 라운드 목록에서 정리한 모바일 우선 카드 레이아웃을 월별 일정, 상태별 보기, 삭제된 라운드 화면에도 동일하게 확장한다.
- 주요 변경:
  - /admin/rounds/calendar 화면을 라운드 목록과 같은 max-w-7xl, 카드형 정보 그리드, 2열/4열 버튼 패턴으로 정리한다.
  - /admin/rounds/status 화면을 같은 카드형 정보 그리드와 버튼 패턴으로 정리한다.
  - /admin/rounds/deleted 화면을 복구 중심 카드 UI로 정리하고 일반 운영 버튼은 표시하지 않는다.
  - 삭제된 라운드는 월별/상태별 일반 목록에서 deleted_at is null 조건으로 계속 제외한다.
- 검증 기준:
  - npm run verify 통과
  - 모바일 폭에서 라운드 정보가 세로로 과도하게 늘어나지 않음
  - 데스크탑 폭에서 라운드 정보가 좁게 눌리지 않음
  - 월별 일정/상태별 보기/삭제된 라운드 화면이 라운드 목록과 같은 레이아웃 흐름을 가짐
