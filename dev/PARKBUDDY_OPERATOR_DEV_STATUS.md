...
## 2026-06-16 - Event to round link flow

### 완료

- 일정 참석 투표(`events`/`event_votes`)를 라운딩 생성(`rounds`/`round_participants`)과 연결하는 1차 흐름을 구현했다.
- `admin_create_round_from_event(p_event_id uuid)` RPC를 추가했다.
- `/schedule`에서 운영자에게만 `참석자 기준 라운딩 생성` 버튼을 표시한다.
- 참석자가 없으면 버튼을 비활성화한다.
- 이미 생성된 일정은 `생성된 라운딩 보기`로 표시해 중복 생성을 방지한다.
- 생성 후 `/admin/rounds/[roundId]/participants`로 이동해 참석자 자동 등록 결과를 확인하도록 했다.
...
1. 라운딩 관리 화면에 연결된 일정 맥락 표시
2. 일정 → 라운딩 생성 완료 UX 보강
3. 생성된 라운딩에서 조 편성/스코어 입력 흐름 확인
