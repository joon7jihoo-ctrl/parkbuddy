...
type ScoringType = 'stroke' | 'new_peoria' | 'match' | 'stableford';
...
function normalizeScoringType(
  ...
  const normalizedValue = value === 'match_play' ? 'match' : value;
...
function getRecommendedGameSettings(totalParticipants: number): {
  ...
  if (totalParticipants >= 8 && totalParticipants % 2 === 0) {
    return {
      ...
      scoringType: 'match',
      message:
        '청백전 + 매치 플레이를 추천합니다. 팀 대항 승점 방식이라 가장 긴장감 있게 즐길 수 있습니다.',
    };
  }
  ...
  if (totalParticipants >= 6) {
    return {
      ...
      scoringType: 'stroke',
      message:
        '스크램블 스트로크를 추천합니다. 실력 차이가 있어도 친선 라운드 분위기를 만들기 좋습니다.',
    };
  }
  ...
  return {
    ...
    message:
      '개인전 신페리오를 추천합니다. 소규모 라운드에서 핸디캡 보정으로 재미를 살릴 수 있습니다.',
  };
}
...
export function RoundPairingForm({
  ...
  const hasSavedAssignments =
    Boolean(defaultAssignments) && Object.keys(defaultAssignments ?? {}).length > 0;
  ...
  const scoringOptions =
    SCORING_OPTIONS_BY_PLAY_MODE[playMode] ?? SCORING_OPTIONS_BY_PLAY_MODE.team_match;

  function handlePlayModeChange(nextValue: string) {
    const nextPlayMode = normalizePlayMode(nextValue);
    const nextOptions =
      SCORING_OPTIONS_BY_PLAY_MODE[nextPlayMode] ?? SCORING_OPTIONS_BY_PLAY_MODE.team_match;
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          ...
          <label className="block">
            ...
            <select
              value={scoringType}
              onChange={(event) =>
                setScoringType(normalizeScoringType(event.target.value, playMode))
              }
              className="mt-2 h-12 w-full rounded-2xl border border-slate-200 px-4 text-slate-900 outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100"
            >
            ...
            {participants.map((participant) => (
              ...
              >
                ...
                <select
                  ...
                  className="h-12 rounded-2xl border border-slate-200 px-3 text-slate-900 outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100"
                >
