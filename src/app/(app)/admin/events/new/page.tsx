...
export default async function NewEventPage() {
  ...
  return (
    <main className="space-y-5">
      <TopBar title="라운딩 공지 등록" />
      <form action={createEvent} className="space-y-4 rounded-3xl bg-white p-5 shadow-sm">
        <label className="block text-sm font-semibold text-slate-700">공지 제목<input name="title" required maxLength={80} className="mt-2 h-12 w-full rounded-2xl border border-slate-200 px-4 outline-none focus:ring-2 focus:ring-emerald-500" /></label>
        ...
        <label className="block text-sm font-semibold text-slate-700">라운딩 일시<input name="starts_at" type="datetime-local" required className="mt-2 h-12 w-full rounded-2xl border border-slate-200 px-4 outline-none focus:ring-2 focus:ring-emerald-500" /></label>
        <label className="block text-sm font-semibold text-slate-700">파크골프장<input name="course_name" required maxLength={80} className="mt-2 h-12 w-full rounded-2xl border border-slate-200 px-4 outline-none focus:ring-2 focus:ring-emerald-500" /></label>
